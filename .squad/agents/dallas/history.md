...
- **2026-04-29T13:05:08-07:00:** Refactored `GroupMatchList.tsx` to use shared `MatchCard` instead of local `GroupMatchCard`. Made `onDelete` and `onEdit` optional in `MatchCard` props — when absent, edit/delete buttons are hidden and `EditMatchDialog` is not rendered. Added optional `subtitle?: string` prop to `MatchCard` for context-specific badge text (e.g. "Logged by Alice"). `GroupMatch extends Match` so passes directly. Group context is read-only for now (no delete/edit handlers passed). Build and TypeScript clean.
 Created 3 hooks (`use-groups.ts`, `use-group-matches.ts`, `use-group-members.ts`) following existing hook patterns (useState + useEffect + useCallback). Created 7 components in `src/components/groups/`: GroupsTab (card-based group list + create CTA + pending invites), GroupView (sub-tabbed detail with matches/members/stats), CreateGroupDialog, InviteMemberDialog (email/name search), PendingInvites, MemberList (role badges, owner remove), GroupMatchList (paginated). Integrated as 6th tab in App.tsx (desktop TabsList grid-cols-6, mobile nav grid-cols-6) using `UsersThree` from phosphor-icons. Added pending invite notification badge (red dot mobile, numbered badge desktop). Modified LogMatchDialog to include a "Log to" Select when user has groups, routing to `logGroupMatch()`. 183 tests pass, build clean. Not pushed to production — feature branch only.
- **2026-04-29T13:29:46-07:00:** Built `DataContextSelector` component (`src/components/shared/DataContextSelector.tsx`) — a Shadcn Select pill for switching between "My Matches" and group data contexts. Wired into PlayersTab and HeroesTab via `dataSource`, `groups`, `dataContext`, `onDataContextChange` props. App.tsx manages per-tab state (`playersContext`, `heroesContext`) and fetches group matches via `useGroupMatches` hook when a group is selected. In group context, HeroesTab hides the "Global Matchup Heatmap" community section. Non-group users see zero UI change (selector returns null when groups.length === 0). TypeScript clean, 183 tests pass, build succeeds.
- **2026-04-29T20:56:00-07:00:** Fixed group invites by implementing shareable invite link system. Email invite had no backend email service — stored in Firestore but never sent. Solution: (1) Added `inviteCode` field to `GameGroup` type. (2) Groups auto-generate 8-char alphanumeric invite codes on creation. (3) `InviteMemberDialog` now shows a "Copy Invite Link" section at top with one-click copy button. (4) App.tsx handles `?join=<code>` URL param — auto-joins group on page load if logged in. (5) Added `getOrCreateInviteCode`, `getGroupByInviteCode`, `joinGroupByInviteCode` to `lib/groups.ts`. Existing in-app search invite still works. TypeScript clean (0 errors), committed to main.

### 2026-04-30T09:43:44-07:00: Session continuity documentation complete
- Created `.squad/identity/now.md` documenting current project state, live features, recent Game Groups completion, testing readiness
- Created `.squad/identity/wisdom.md` with hard-won knowledge: tab-based SPA patterns, component extraction patterns, shared component design, DataContextSelector patterns, group invite best practices
- All decisions consolidated to canonical `.squad/decisions.md`
- Documentation ready for next session instantiation

- **2026-04-30T10:41:08-07:00:** Added `DataContextSelector` to MatchesTab, following the exact PlayersTab/HeroesTab pattern. App.tsx: added `matchesContext` state, `useAllGroupMatches` hook call, `matchesDataSource` memo, passed 4 new props to MatchesTab. MatchesTab: renders selector before header, computes `effectiveMatches` from `dataSource?.matches ?? matches`, uses spread for conditional onEdit/onDelete (group = read-only), shows "Logged by {name}" subtitle on group match cards. Title stays "Match History" in all contexts per Devin's directive. "Log Match" button remains visible (LogMatchDialog has its own group selector). Non-group users see zero change (selector returns null). Build passes, only pre-existing test fixture error (`turnOrder` missing in MatchesTab.test.tsx).
